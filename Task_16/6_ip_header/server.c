...
int main(void) {
    ...
    while (1) {
        ...
        int bytes_recv = recvfrom(sock_fd, datagram, sizeof(datagram), 0, (struct sockaddr *)&client, &client_len);
        if (bytes_recv == 0) {
        ...
        int bytes_send = sendto(sock_fd, datagram, sizeof(datagram), 0, (struct sockaddr *)&client, client_len);
        if (bytes_send < 0) {
