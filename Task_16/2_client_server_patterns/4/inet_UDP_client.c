...
int main(void) {
    ...
    if (host_info == NULL) {
        perror("failet to get host information");
        close(sock_fd);
        exit(EXIT_FAILURE);
    }
    ...
    if (sendto(sock_fd, "1", 1, 0, (struct sockaddr *)&server, server_len) == -1) {
        perror("failed to sending request to server");
        close(sock_fd);
        exit(EXIT_FAILURE);
    }
    
    // чтение данных из сокета
    ssize_t bytes_recv = recvfrom(sock_fd, (void *)&tv, sizeof(tv), 0, (struct sockaddr *)&server, &server_len);
    if(bytes_recv == -1) {
