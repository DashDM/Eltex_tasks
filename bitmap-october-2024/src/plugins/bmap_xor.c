...
/**
 * @brief Performs a bitwise XOR operation on two bitmaps.
 *
 * This function creates a new bitmap that is the result of performing
 * a bitwise XOR operation on the bits of the `first` and `second`
 * bitmaps. The resulting bitmap will have bits set to 1 where the bits
 * of the input bitmaps differ (i.e., one is 1 and the other is 0).
 *
...
struct bitmap *bmap_xor(struct bitmap *first, struct bitmap *second) {
  log_msg("Starting XOR operation. First_bm: %p, Second_bm: %p",
  ...
  log_msg("Performing bitwise XOR operation.");
  ...
  log_msg("Bitwise XOR operation completed successfully.");
  return result;
}
