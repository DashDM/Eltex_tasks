...
START_TEST(bmap_xor_1) {
  ...
  struct bitmap *res = bmap_xor(bm1, bm2);

  ck_assert_int_eq(res->bits[0], 0b00111100);
  ck_assert_int_eq(res->bits[1], 0b01010001);
...
START_TEST(bmap_xor_3) {
  ...
  struct bitmap *res = bmap_xor(bm2, bm1);
...
Suite *suite_bmap_xor(void) {
  Suite *s = suite_create("\033[45m-=BMAP_XOR=-\033[0m");
  TCase *tc = tcase_create("bmap_xor");

  tcase_add_test(tc, bmap_xor_1);
  tcase_add_test(tc, bmap_xor_2);
  tcase_add_test(tc, bmap_xor_3);
  tcase_add_test(tc, bmap_xor_4);
